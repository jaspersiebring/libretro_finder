...
class Test_recursive_hash():
    def test_with_fixture(self, setup_files: Tuple[pathlib.Path, pd.DataFrame]) -> None:
        ...
        file_names = np.array([file_path.relative_to(bios_dir).as_posix() for file_path in file_paths])
        assert np.all(np.isin(file_names, bios_lut['name'].values)) # type: ignore

        index_a, index_b = np.where(file_names.reshape(1, -1) == bios_lut['name'].values.reshape(-1,1))
    ...
    def test_without_fixture(self, tmpdir_factory: TempdirFactory) -> None:
        ...
        output_paths, output_hashes = recursive_hash(directory=temp_output_dir, glob="*")
...
class Test_match_arrays():
    def test_overlap(self):
        ...
        matching_values, indices_a, indices_b = match_arrays(array_a=array_a,array_b=array_b)
