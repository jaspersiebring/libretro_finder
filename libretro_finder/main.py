...
from libretro_finder.utils import recursive_hash, match_arrays
from typing import List


def organize(search_dir: pathlib.Path, output_dir: pathlib.Path, glob: str = "*") -> None:
    """
    ...
    assert np.array_equal(system_subset['md5'].values, hashes)
    ...
    print(f"{matches['name'].sum()} matching BIOS files were found for {matches.shape[0]} unique systems:")
